...
void usb_device_init(void)
{
    ...
    USBx->GINTMSK  = 0UL;
    USBx->GINTMSK |= USB_OTG_GINTMSK_USBRST; // USB Reset interrupt
    USBx->GINTMSK |= USB_OTG_GINTMSK_RXFLVLM; // RX FIFO Non-Empty (Receive FIFO Level) interrupt
    USBx->GINTMSK |= USB_OTG_GINTMSK_IEPINT; // IN Endpoint Interrupt
    USBx->GINTMSK |= USB_OTG_GINTMSK_OEPINT; // OUT Endpoint Interrupt
...
static void ep0_init_after_reset(void)
{
    ...
    USBx_OUTEP(0)->DOEPTSIZ = (3UL << 29) | (1UL << 19) | 8UL; // SUPCNT=3, PKTCNT=1, XFRSIZ=8
